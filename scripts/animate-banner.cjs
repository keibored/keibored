...
async function main(){
 ...
 for(let frame=0;frame<N;frame++){
  ...
  for(let y=0;y<H;y++)for(let x=0;x<W;x++){
   let dx=0,dy=0;
   // Small fold ripples only inside opaque fabric; never move a whole rectangle.
   dx+=cloth[y*W+x]*1.5*Math.sin(phase+y/76+(x>300?0.8:0))*smooth(0,110,y);
 ...
 execFileSync('ffmpeg',['-v','error','-y','-framerate','20','-i',path.join(frames,'%03d.png'),'-filter_complex','split[a][b];[a]palettegen=stats_mode=diff[p];[b][p]paletteuse=dither=bayer:bayer_scale=3:diff_mode=rectangle','-loop','0',path.join(root,'assets/kei-banner-stack-v3.gif')],{stdio:'inherit'});
